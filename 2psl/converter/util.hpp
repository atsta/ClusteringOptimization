...
class Node 
{
    public:
        uint32_t i;
        std::map<uint32_t,node_t> communityDegrees;
        Node() : i(0) {}
        Node(uint32_t id) : i(id) {}
        node_t getDegrees(uint32_t community) {
            if (communityDegrees.count(community) > 0)
                return communityDegrees.find(community)->second;
            else 
                return 0;
        }
        void updateDegrees(uint32_t community, node_t value) 
        {
            if (value < 0)
                value = 0;
